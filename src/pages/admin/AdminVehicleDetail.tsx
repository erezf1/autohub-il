    ...
    <div className="space-y-6 min-h-full">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-white hebrew-text">
              {vehicle.make?.name_hebrew} {vehicle.model?.name_hebrew} {vehicle.year}
            </h1>
            <p className="text-white/70 hebrew-text">מזהה רכב: #{id}</p>
          </div>
        </div>
          ...
          <CardContent className="pt-6" dir="rtl">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 text-right">
              <div>
                ...
                <p className="text-2xl font-bold text-white hebrew-text">₪{vehicle.price?.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground hebrew-text">קילומטרז׳</p>
                <p className="text-lg font-semibold text-white hebrew-text">{vehicle.kilometers?.toLocaleString()}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground hebrew-text">שנה</p>
                <p className="text-lg font-semibold text-white hebrew-text">{vehicle.year}</p>
              </div>
              <div>
                ...
                <div className="flex gap-2 items-center">
                  ...
                  {vehicle.is_boosted && vehicle.boosted_until && new Date(vehicle.boosted_until) > new Date() && (
                    <Badge className="flex gap-1 bg-amber-500 text-black">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-right mt-4">
              <div>
                ...
                <p className="text-lg font-semibold text-white hebrew-text">{getVehicleTypeLabel(vehicle.sub_model || 'car')}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground hebrew-text">בעלים</p>
                <p className="text-lg text-white hebrew-text">{vehicle.owner?.business_name || vehicle.owner?.full_name || '-'}</p>
              </div>
              <div>
                <p className="text-sm text-muted-foreground hebrew-text">תאריך הוספה</p>
                <p className="text-lg text-white hebrew-text">{formatDistanceToNow(new Date(vehicle.created_at), { addSuffix: true, locale: he })}</p>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="details" className="w-full">
        ...
        <TabsContent value="details" className="space-y-4">
              ...
              <CardContent dir="rtl">
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-right">
                  <div>
                    ...
                    <p className="font-medium text-white hebrew-text">{vehicle.make?.name_hebrew}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground hebrew-text">דגם</p>
                    <p className="font-medium text-white hebrew-text">{vehicle.model?.name_hebrew}</p>
                  </div>
                  {vehicle.sub_model && (
                    <div>
                      <p className="text-sm text-muted-foreground hebrew-text">סוג</p>
                  ...
                  <div>
                    ...
                    <p className="font-medium text-white hebrew-text">{getFuelTypeLabel(vehicle.fuel_type)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground hebrew-text">תיבת הילוכים</p>
                    <p className="font-medium text-white hebrew-text">{getTransmissionLabel(vehicle.transmission)}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground hebrew-text">נפח מנוע</p>
                    <p className="font-medium text-white hebrew-text">{vehicle.engine_size ? `${vehicle.engine_size}L` : '-'}</p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground hebrew-text">בעלים קודמים</p>
                    <p className="font-medium text-white">{vehicle.previous_owners || 1}</p>
                  </div>
          ...
          {vehicle.test_result_file_url && (
            <GradientBorderContainer className="rounded-md">
              <Card className="bg-black border-0 rounded-md">
                <CardHeader>
                  <CardTitle className="text-white hebrew-text">תוצאות טסט</CardTitle>
                </CardHeader>
                <CardContent>
                  <GradientBorderContainer className="rounded-md inline-block">
                    <Button 
                      ...
                      className="hebrew-text border-0 bg-black rounded-md hover:bg-gradient-to-r hover:from-[#5be1fd] hover:to-[#2277ee] hover:text-black transition-all"
                    >
          ...
          {vehicle.description && (
            <GradientBorderContainer className="rounded-md">
              <Card className="bg-black border-0 rounded-md">
                <CardHeader>
                  <CardTitle className="text-white hebrew-text">תיאור</CardTitle>
                </CardHeader>
                <CardContent dir="rtl">
                ...
                <CardContent dir="rtl">
                  <div className="flex flex-wrap gap-2 justify-end">
                    {vehicle.tags.map((tag: any) => (
                      <Badge key={tag.id} variant="secondary" style={{ backgroundColor: tag.color }} className="text-black">
                        {tag.name_hebrew}
                      </Badge>
                    ))}
                  </div>
        ...
        <TabsContent value="seller">
          <GradientBorderContainer className="rounded-md">
            <Card className="bg-black border-0 rounded-md">
              <CardHeader>
                <CardTitle className="text-white hebrew-text">מידע על המוכר</CardTitle>
              </CardHeader>
        ...
        <TabsContent value="activity" className="space-y-4">
          <GradientBorderContainer className="rounded-md">
            <Card className="bg-black border-0 rounded-md">
              <CardHeader>
                <CardTitle className="text-white hebrew-text">פעילות אחרונה</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b">
                    <span className="text-white hebrew-text">הרכב נוסף למערכת</span>
        ...
        <TabsContent value="images" className="space-y-4">
          <GradientBorderContainer className="rounded-md">
            <Card className="bg-black border-0 rounded-md">
              <CardHeader>
                <CardTitle className="text-white hebrew-text">גלריית תמונות</CardTitle>
              </CardHeader>
              <CardContent>
