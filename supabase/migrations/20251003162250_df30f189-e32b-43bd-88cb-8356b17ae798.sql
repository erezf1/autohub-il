...
-- WITH SECURITY IMPROVEMENTS (roles in separate table)
-- ============================================
...
CREATE TYPE public.app_role AS ENUM ('dealer', 'admin', 'support');
...
-- Profile and role creation trigger
...
-- Helper function using has_role
...
DROP POLICY IF EXISTS "Active users insert vehicles" ON public.vehicle_listings;
CREATE POLICY "Active users insert vehicles" ON public.vehicle_listings
...
DROP POLICY IF EXISTS "Active users place bids" ON public.auction_bids;
CREATE POLICY "Active users place bids" ON public.auction_bids
  FOR INSERT WITH CHECK (
    bidder_id = auth.uid() AND
    EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND status = 'active')
  );

DROP POLICY IF EXISTS "Users see bids" ON public.auction_bids;
CREATE POLICY "Users see bids" ON public.auction_bids
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND status = 'active')
  );

-- ISO request policies
DROP POLICY IF EXISTS "Users manage own ISO requests" ON public.iso_requests;
CREATE POLICY "Users manage own ISO requests" ON public.iso_requests
  FOR ALL USING (requester_id = auth.uid());

DROP POLICY IF EXISTS "Active users see ISO requests" ON public.iso_requests;
CREATE POLICY "Active users see ISO requests" ON public.iso_requests
  FOR SELECT USING (
    status = 'active' AND
    EXISTS (SELECT 1 FROM public.users WHERE id = auth.uid() AND status = 'active')
  );

DROP POLICY IF EXISTS "Active users insert ISO requests" ON public.iso_requests;
CREATE POLICY "Active users insert ISO requests" ON public.iso_requests
  FOR INSERT WITH CHECK (
...
CREATE POLICY "Users create conversations" ON public.chat_conversations
  FOR INSERT WITH CHECK (
...
DROP POLICY IF EXISTS "Users send messages in their conversations" ON public.chat_messages;
CREATE POLICY "Users send messages in their conversations" ON public.chat_messages
  FOR INSERT WITH CHECK (
