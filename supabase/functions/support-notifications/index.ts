...
interface NotificationPayload {
  type: 'vehicle_reported' | 'user_report' | 'high_priority_ticket' | 'auction_dispute' | 'user_password_reset';
  entityId: string;
  entityType?: string;
  title: string;
  description: string;
...
serve(async (req) => {
  ...
  try {
    ...
    // Validate payload
    if (!payload.type || !payload.entityId || !payload.title || !payload.description) {
      return new Response(
    ...
    console.error('Error in support-notifications:', error);
    return new Response(
